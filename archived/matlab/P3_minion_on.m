...
wind_frequency = [0.000000000000000000001, 0.3];        % Frequency of sinusoidal variation (Hz)
wind_noise_amplitude = [8, 0.02];% Amplitude of random noise
wind_record = zeros(length(time), 2);

% dropped_package = false;
drop_down_distance = 0;

% Simulation loop
for t = 1:length(time)
    % Dynamic wind force (sinusoidal + random noise)
    wind_force(1) = wind_base(1) + wind_amplitude(1) * sin(2 * pi * wind_frequency(1) * dt * time(t)) ...
...
video = VideoWriter('6-funky.mp4', 'MPEG-4');
