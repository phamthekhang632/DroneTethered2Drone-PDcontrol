...
for t = 1:length(time)
    ...
    F1 = (thrust / 2) + 2*(torque_input / minion_l);
    F1 = min(max(F1, 0), minion_max_thrust);
    F2 = (thrust / 2) - 2*(torque_input / minion_l);
    F2 = min(max(F2, 0), minion_max_thrust);

    thrust_force = (F1+F2) * [sin(minion_theta), cos(minion_theta)];
...
title('Thrust Forces F1 and F2 Over Time (failed attempt)');
xlabel('Time (s)');
