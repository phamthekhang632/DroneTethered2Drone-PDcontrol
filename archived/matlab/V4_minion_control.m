...
masses = [10, repmat(default_mass, 1, N-2), 1]; % Masses: Mothership (10kg), Minion (1kg), Others (0.05kg)
...
for t = 1:length(time)
    ...
    % forces(N, 1) = forces(N, 1) + pd_force_minion_x;      % Apply horizontal thrust for the minion only

    % Apply the PD control force with the maximum force constraint
...
for t = 1:10:length(time)
    ...
    plot(target_position(1), target_position(2), 'rx', 'MarkerSize', 10, 'LineWidth', 2, 'DisplayName', 'Target Position');
