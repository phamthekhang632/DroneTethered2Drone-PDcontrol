...
rope_angle = deg2rad(100);         % Angle of the rope with respect to the horizontal (in radians)
...
for t = 1:length(time)
    ...
    thrust = masses(N) * (control_input(1)) / cos(minion_theta(t));
    torque_input = minion_I * control_input(2);

    F1 = (thrust / 2) + 2*(torque_input / minion_l)*minion_max_thrust;
    F1 = min(max(F1, 0), minion_max_thrust);
    F2 = (thrust / 2) - 2*(torque_input / minion_l)*minion_max_thrust;
    ...
    torque_actual = (F2 - F1) * (minion_l / 2);
    minion_alpha = torque_actual / minion_I;

    if t==1
        minion_omega(t) = minion_alpha * dt;
        minion_theta(t) = minion_theta(t) + minion_omega(t) * dt;
    else 
        minion_omega(t) = minion_omega(t-1) + minion_alpha * dt;
        minion_theta(t) = minion_theta(t-1) + minion_omega(t-1) * dt;
    end
...
hold off;
%%
% Plot F1 and F2 over time
figure;
plot(time, minion_thrust(:, 1), 'b', 'LineWidth', 2, 'DisplayName', 'F1 Thrust');
hold on;
plot(time, minion_thrust(:, 2), '--r', 'LineWidth', 2, 'DisplayName', 'F2 Thrust');
title('Thrust Forces F1 and F2 Over Time');
xlabel('Time (s)');
...
figure;
plot(time, rad2deg(minion_theta), 'g', 'LineWidth', 2, 'DisplayName', 'Pitch Angle');
